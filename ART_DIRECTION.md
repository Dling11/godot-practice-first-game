...
The active Sanctuary establishes inhabited fantasy through a generated-and-normalized walk-in angel portal, separate divine fountain, compact violet skillkeeper lodge, warm armskeeper workshop and low cart, two tree silhouettes, Skillkeeper Eira, and Armskeeper Orren. Dedicated restrained grass and cobblestone transitions replace borrowed combat-stage terrain. The fountain remains physically separate from the portal so its walk-around footprint, visible courtyard gap, broad stairs, and open doorway read at gameplay scale. Eira and Orren use Sanctuary-compatible oversized heads, narrow tiny bodies, tiny feet, and two cleanly detached role props rather than tall anatomical sprites. Service structures use larger deliberate clusters, clear emblems, complete silhouettes, and no baked-in character bodies. Portal runes, water glints, magic accents, lanterns, windows, tree sway, and one-pixel NPC breathing animate independently from collision and interaction so the hub feels alive without becoming a noisy cartoon town.
