...
- Earthsplitter Foundation review: `assets/vfx/abilities/king/earthsplitter/sword.png` (4x4, padded 512px cells) (sixteen approved drawings, unchanged) plus active `pressure_v2.png` (8x5, 256px cells, forty drawings), binary alpha. The original sixteen-frame `rupture.png` is active again as upright eruption stamps after the thin pressure-only composition was rejected; `pressure_v2.png` now supplies short embedded flashes instead of a lone traveling spark. Pressure rows are right/down/up/down-right/up-right; horizontal mirrors supply left counterparts. `tools/build_earthsplitter_pressure.gd` packs only the new ground sheet; `PROMPTS_V2.md` records the built-in generation prompt. Built-in generation sources/prompts: `art_source/generated/vfx/king/earthsplitter/`; registration/extraction: `tools/build_earthsplitter_review.gd`. Sword registration follows grip during windup and tip at contact; horizontal facing mirrors instead of inverting the overhead swing. Reuses existing greatsword cleave and Riftbreak slam audio, with tuned gain/pitch. Lab only; no external pack imported.

- Riftbreak targeted review VFX: `assets/vfx/abilities/king/riftbreak_review/rupture_256.png`, 4x4 cells, 256px each, 16 distinct drawings. Binary alpha, fixed (128,128) contact; SpriteFrames has three anticipation drawings, twelve impact/debris drawings and one residual. Source/prompts: `art_source/generated/vfx/king/riftbreak_review_2026_09_14/`; reproducible Godot packer: `tools/build_riftbreak_review.gd`. Used only by the explicit Lab comparison.
