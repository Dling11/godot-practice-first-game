...
- The Examiner debug proof owns six exact-grid `192x128` compact-pixel runtime sheets under `assets/characters/enemies/examiner/`: locomotion, thrust, sweep, Zero Interval, Refutation, and reaction/withdrawal. Approved generated boards and prompt provenance live under `art_source/generated/characters/disciples/examiner/compact_pixel_v2/`; `tools/process_examiner_animation_sources.py` removes preview mattes, finds real gutters, applies one fixed scale, anchors the lower body to a common foot line, derives the symmetrical right-facing row from left by exact mirroring, and protects every cell from clipping. Axiom composes the clean sweep/dash poses and keeps lane presentation separate. V2 gameplay captures live under `art_source/review/characters/disciples/examiner/`.
