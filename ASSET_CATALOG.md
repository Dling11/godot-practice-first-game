...
- Status: `production_runtime`; identity, complete encounter, entrance conversation, audio, and Decision 097 milestone reward are active. Canonical runtime copies live under `assets/characters/enemies/stage_5_boss/`; F8 remains an isolated fallback arena. The final display name is Varkuun, Lord of the Withered Grove. His active CC0 fantasy/JRPG loop is `assets/audio/music/boss/varkuun_battle_rpg_theme_loop.ogg`, and his complete real-recording action suite lives under `assets/audio/sfx/enemies/varkuun/`.
...
| `portrait_king` | `art_source/generated/characters/playable/king/simple_reboot/king_portrait_source_v1.png` | `assets/characters/playable/king/portraits/king_portrait_96x96.png` | Identity-matched 96x96 transparent dialogue portrait | `active_runtime` |
...
| `prop_varkuun_chest_closed` | `assets/gameplay/loot/stage_clear_chest/varkuun_chest_closed_74x66.png` | `active_runtime` | Closed 74x66 oxblood/plum, dead-root, and iron Stage V boss chest; exactly two pixels larger per axis than the Stage III Reliquary. |
| `prop_varkuun_chest_open` | `assets/gameplay/loot/stage_clear_chest/varkuun_chest_open_74x66.png` | `active_runtime` | Open 74x66 Varkuun chest claim-success state with restrained gold/green inner light. |
| `material_forest_varkuun_core` | `assets/items/materials/forest/varkuun_core_24x24.png` | `active_runtime` | Boss-rarity catalyst icon derived from the chest's central seal. |
...
Varkuun's Chest and King's portrait were generated with the built-in image tool from the approved Rootbound chest/Varkuun and King identity references. The owner-directed v2 chest edit preserves the approved construction while adding restrained oxblood/plum high-tier panels and aged gold detail. `tools/process_stage_5_reward_assets.py` removes the board's connected light background, normalizes both chest states to stable grounded 74x66 canvases, emits King's transparent 96x96 portrait, and derives the 24x24 Varkuun Core icon from the chest seal. Generated sources remain under matching `art_source/generated/` paths.
