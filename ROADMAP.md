...
- Refactored Rootbound Husk's attacks into a dedicated data profile: a quick single Root Spear, a slower three-lane Root Fan whose center erupts before its sides, and a readable below-half-health cadence increase. Damage, health, snapshotted hitboxes, and the four-enemy cap remain unchanged; one attack-start event owns sound while staged eruption events own VFX.
...
- Added Stage 3 Ashen Pilgrimage with a direct Sanctuary route, a three-enemy approach, and the first solo Rootbound Husk mini-boss. Its Boss-tier rooted spear/three-lane volley foundation and CC0 boss track are active, while the four-enemy cap remains unchanged.
