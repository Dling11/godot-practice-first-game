...
| Select / engage enemy | Left-click once to select; double-click to auto-engage; ground click clears and moves | Not assigned |
| Primary attack: equipped sword | 1 or click the HUD Attack button; swings freely without a nearby selected target | Right trigger |
...
| Skill 1: Echoing Sever targeting | 2 or click its HUD slot | Left shoulder |
| Confirm Echoing Sever | Left mouse | Right trigger |
| Cancel Echoing Sever | Right mouse or Escape | UI Cancel |
| Skill 2: Riftbreak self-AOE | 3 or click its HUD slot | Reserved |
| Skill 3: Sovereign Pursuit ground leap | 4, aim, then left-click/right-trigger | Right trigger confirms |
| Skill 4: spirit-sword target and delayed AOE | 5, aim, then left-click/right-trigger | Right trigger confirms |
...
Movement, movement-owned facing, numbered Attack, click movement, optional enemy engagement, dash, portal interaction, and arena restart after defeat are active for the temporary King proof. Single-click selects, double-click auto-engages, and clicking ground clears combat and moves. `1` Attack swings freely unless a selected enemy is within the 260-pixel assist radius. One latest valid attack, dash, or equipped skill input is retained for at most 0.8 seconds and executes only at the first legal recovery or ability-finished boundary; it never waits through a cooldown. Balanced Slash supplies the tested 58-pixel-forward by 96-pixel-wide contact fan, while King's integrated sword and white-blue trail present the hit. Echoing Sever is Skill 1 on `2`; aim its smooth exact-angle wedge, then left-click/right-trigger to commit. Riftbreak is Skill 2 on `3` and immediately slams one 84-pixel circle around King's feet. Sovereign Pursuit is Skill 3 on `4`; aim and confirm its ground leap, then `3` may buffer Riftbreak until Pursuit finishes. Skill 4 is on `5`; aim and confirm its concentrated spirit-sword crash and delayed AOE explosion. Opaw's preserved skills are not silently attached to King. The shared proof vitality still begins at 140, gains 12 per level, retains health across stages, and regenerates 1 HP/s after five damage-free seconds. F9 grants level, coins, authored gear, maximum materials, and session-only unlimited skill cooldowns without changing King's normal balance data or skill loadout.
