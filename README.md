...
| Debug test loadout | F9 (debug builds: level 10, 999 coins, unlimited skill cooldowns, authored skills/gear, maximum materials, Admin review tools) | Not assigned |
| Toggle Admin Mode | F10 (debug builds only; reveals Sanctuary Admin Tools) | Not assigned |
| Stage VI environment preview | F6 while Admin Mode is enabled | Not assigned |
...
In a debug build, F9 enables the complete non-saving review preset and reveals Admin Tools; F10 remains the manual Admin Mode toggle. From Sanctuary, click `STAGE VI LOOK [F6]` in that panel or press F6 to open the non-campaign `The Elder Ascent` environment preview. It is deliberately not an expedition-list campaign route yet and has no enemies, rewards, completion, or save authority. Use `COMBAT LAB [F7]` or F7 for the lab. The lab selects the Stage 5 boss proof by default and can spawn 1/4/8 of any current mob, Elite, mini-boss, or boss; pause enemy AI; toggle King invincibility; enable unlimited authored skills; clear; or reset. The lab removes enemy reward components before spawning and suppresses autosave, so lab combat cannot grant XP, coins, materials, story progress, or stage claims.
