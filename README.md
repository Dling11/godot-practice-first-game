...
**Earthsplitter Skill 1 review:** F7 -> KING REVIEW -> SKILL 1: ORIGINAL toggles EARTHSPLITTER. Press **1**, aim the fixed-length cyan lane, then left-click; right-click/Esc cancels aim. This is the separate-sword Foundation proof with .36s base cast, moving rupture and no stun. Toggle off to restore the previous skill/loadout. Nearby/far cursor positions give the same reach; terrain shortens the lane with an amber end marker. [Current fuller ground-eruption review](art_source/review/characters/king/earthsplitter_eruption_2026_09_14/index.html). Direct launch: `res://levels/combat_lab/combat_lab.tscn -- --earthsplitter-review`. Use **FORM: FOUNDATION** to switch to **ADVANCED** (Cascading Rupture): three quick waves, a longer stronger third, and the same .36s cast. Direct Advanced launch: `res://levels/combat_lab/combat_lab.tscn -- --earthsplitter-advanced-review`. Form switching waits for the released waves to finish. [Advanced playback with audio](art_source/review/characters/king/earthsplitter_advanced_2026_09_14/earthsplitter_advanced.mp4). Later forms, stage unlocks/save integration and the new Molten Crash remain pending.
...
### Targeted Riftbreak review

F7 -> KING REVIEW -> **RIFTBREAK: ORIGINAL** toggles **TARGETED TEST**. Press **2**, aim, and left-click to confirm; right-click/Esc cancels. The inner marker identifies the stun center; the outer blast only flinches/pushes. Control returns in 0.38 seconds before impact pauses while debris continues. Toggle off to restore the original skill and previous loadout. This is a session-local comparison. HIT and STUN now explicitly test different reactions.
