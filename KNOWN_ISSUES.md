...
- **Status:** Open
- **Progress:** Decisions 077-082 temporarily make King the live player with tested locomotion, basic slash, and four playable skills. His down-facing eyes are now separated from the black fringe through a deterministic pixel-local correction with no atlas/anchor changes. Skill 4 proves separate eight-frame generated sword and ground sequences, two authoritative radii/damage windows, original audio, corrected shared impact anchoring, and world-locked residue. The live and isolated preview structures each use one body `AnimatedSprite2D`; Opaw remains a complete explicitly tested bench package. A first shared Forest core-gear concept board exists for review only.
...
- **Status:** Open
- **Progress:** Segments 1-4 plus Decision 097 provide safe-point Save/Continue, thirteen illustrated materials, protected enemy drops, direct Stage I-II banking, Stage III's Rootbound Reliquary, and Stage V's distinct Varkuun chest. Its first clear grants the persisted core-gear seal and two Varkuun Cores; replay grants one core. Nema's Living Rootforge remains a safe read-only preview.
- **Impact:** Players can earn, retain, and inspect Forest materials and the Stage V seal, but cannot spend them or produce/equip the four data-only outputs through normal play. No `CraftingService`, output equipment, Hunt selector, Stage VIII standard-accessory seal, Stage X relic/signature seal, or regional Mastery exists.
...
- **Workaround:** Play through Stage V to collect materials and the core-gear seal, then inspect them in Character & Bag or Nema's Living Rootforge. Treat them as saved preparation inventory until crafting transactions are implemented; F9 samples remain debug-only and non-saving.
