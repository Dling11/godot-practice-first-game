...
func _run() -> void:
	...
	if (
		...
		return
	if menu.route_buttons.size() != 5 or menu.first_expedition_button.disabled:
		_fail("The expedition menu did not build one available and four sealed data-driven routes.")
		return
	if (
		...
		or not menu.route_buttons[4].disabled
	):
