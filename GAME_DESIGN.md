...
**Implemented early weapon inventory:** Ashwood Blade is the always-owned, unsellable **Wood** fallback. Orren's **Iron Sword** costs 90 coins, deals 32 damage with 56 knockback, and reuses Balanced Slash plus Opaw's existing weaponless body animation. Stage I's full 46-coin reward cannot buy it; the implemented forest arc offers 144 coins total, making Iron a later earned milestone instead of an immediate purchase. Purchasing stores Iron without auto-equipping it. Orren's owned-item action and Gear & Armory's detail panel both expose an explicit Equip button; selecting a card only previews it. A valid equip synchronizes the authoritative `WeaponDefinition`, world sprite, menu portrait, damage, and knockback at the next safe idle boundary. Opaw's Warrior class rejects weapons whose `compatible_classes` omit `warrior`. Ownership and equipped choice autosave in Sanctuary. The current beginner set deliberately stops at Wood and Iron; spirit-blue higher grades remain deferred. Drops, armor stats, and selling are not implemented.
