...
**Implemented Forest economy data and inventory-view foundation:** the stable current material set is Mire Resin/Mire Membrane from Mirelings, Root Fiber/Young Heartwood from Rootlings, Forsaken Cloth/Weathered Fittings from Thralls, Barbed Seed/Thorn Sap from Bramble Spitters, and Husk Heartwood/Rootbound Core from the Rootbound Husk. Validated data now defines those materials, each current enemy's guaranteed/optional/boss reward plan, Stage I-III first-clear and replay chest plans, and four deterministic blueprint contracts: Rootfiber Wraps, Mireward Charm, Thornward Clasp, and Huskbound Guard. Material quantities and discovered recipe IDs have dedicated saved authorities. Nonzero material stacks can now be inspected in Character & Bag, using temporary family glyphs where final icons are absent. The definitions still do not grant rewards, spawn chests, craft items, or create usable equipment.

**Approved next Forest economy behavior:** enemy defeats will feed the authored ecology-linked material pools, every completed stage will produce a non-empty clear chest, and first-clear blueprint/material grants will be idempotent. Leather begins only when a visible hide-bearing Stage 4 creature exists. A separate Rootweaver/Grove Artificer service will own crafting presentation; Orren remains the weapon merchant. Initial quantities are authored for implementation testing and may receive later balance tuning, but drops must continue to match each creature's body, habitat, and combat behavior.
