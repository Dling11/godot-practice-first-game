...
- **Implemented input commitment:** One latest valid normal attack or immediate-directional skill may buffer through a committed normal attack or active dash and starts at the first vulnerable recovery boundary. A living hit window is never interrupted and inputs do not stack. Once a normal attack is accepted, its direction and sword-hitbox pivot remain locked even if the player immediately presses the opposite movement direction; the newest movement facing applies only after recovery. Skills remain committed unless their data explicitly permits dash cancellation; Consecutive Thrust is the first such exception.
...
**King animation/targeting direction:** Decision 076 replaces the rejected detailed greatsword package with a simple Opaw-complexity black-haired swordsman: plain face, crimson scarf, compact navy/charcoal body, mitten hands, tiny feet, and one short broad straight signature sword. Movement targets four purposeful contact/passing poses per direction; attacks keep body motion small and rely on rigid weapon and separate VFX presentation. Narrow line/wedge/cone or actor-dependent skills enter explicit aim-preview mode before commitment. Ground skills expose a movable valid-range marker. Cancel spends no cooldown/resource; confirm snapshots the accepted target. Instant activation is reserved for self-AOE, aura, buff, defense, or naturally broad waves. Targeting, range clamping, collision validation, invulnerability, damage, and cooldown remain gameplay authority; frames present accepted state only.

**Implemented prototype vitality/defeat loop:** Opaw begins with 140 maximum health and gains 12 maximum health per level, reaching 248 at Level 10 before equipment. A level increase grants the new capacity while preserving existing missing damage, then gives Opaw a brief low-opacity gold/spirit glow, a small rising `LEVEL N` label above his head, and a short chime. It never pauses play or places a panel over combat. Current HP survives portal travel, Sanctuary returns, and safe-point Continue. After five damage-free seconds, baseline regeneration restores only 1 HP per second through `HealthComponent`; incoming damage restarts the delay, and full health or defeat stops it. `PlayerVitalityComponent` aggregates character/level/future flat-armor maximums, `PlayerHealthRegenerationComponent` schedules recovery, and `HealthComponent` remains heal/damage/death authority. At zero health, movement and combat stop, Opaw enters a fallen presentation, and restart resets the active run to full health. Armor items, lifesteal, critical stats, potions, and mana remain unimplemented.

**Implemented combat feedback:** an accepted outgoing hit turns each enemy silhouette white for 0.10 seconds and adds a short gold damage number plus a white-hot burst core with three colored pixel sparks. A normal swing shares one restrained camera nudge, positional impact cue, and non-stacking 0.025-second impact freeze across all contacted targets; Consecutive Thrust's finisher uses a separate 0.04-second heavy tier. The normal Ashwood strike delivers light 48 px/s knockback while Sweeping Cut retains its stronger 90 px/s spacing push. Accepted incoming player damage displays red numbers and bursts with sound and camera response, but does not freeze gameplay. Misses, blocked hits, and telegraphed attacks produce none of this confirmed-hit feedback.
...
The active runtime player is **Opaw**, presented as a **Mortal Wayfarer** and **Novice Warrior**. Decision 072 preserves him, his compact-armless presentation, and his current skills as supported roster content. The planned narrative lead is **King**, a young-prime isekai swordsman who died alongside his family and climbs the divine portal sequence seeking the fate of their souls. `Player` remains the technical gameplay role so both characters and a future switchable roster can reuse authority contracts without turning generic code into character-specific code.

King establishes the character-owned combat direction with a compact simple hard-pixel body, one fixed short broad signature sword, a real tap/hold attack chain, four character skills, and eventual cinematic presentation. Decision 076 supersedes the detailed Decision 073-074 body/greatsword clauses: the approved identity uses chunky black hair, a plain two-eye face, crimson scarf, navy/charcoal clothing, mitten-block hands, and tiny feet at Opaw-like production complexity. Equipment becomes essence/relic power that modifies stats, traits, and supported accents without replacing the visible signature sword. Future characters still own their body frames, contact silhouettes, skills, and story motives rather than acting as cosmetic skins. Opaw remains active beside King; roster switching is unimplemented and belongs in Sanctuary or another safe preparation space.
...
**Implemented starter weapon - Ashwood Blade:** Opaw carries a small wooden sword as a separate world presentation sprite. Its `WeaponDefinition` supplies the texture's grip offset, visual scale, swing radius, and authoritative melee shape so the weapon rotates around its authored grip and future weapon families can define honest reach. In the front view the hilt connects at the left torso edge and the blade rises diagonally outward toward screen-left instead of crossing Opaw's head; side grips are raised and held closer to his body. The presentation observes authoritative wind-up, active, and recovery phases, while the weaponless body supplies a deep coil, explosive lunge, and overshoot at fixed scale. The normal attack uses a 0.11-second anticipation, fast 0.08-second committed sweep, mirrored arc, forward extension, dense white-gold inner trail, translucent white-blue cleave band, and 0.21-second recovery. Balanced Slash's reusable beginner-sword fan reaches 58 pixels forward and 48 pixels to either side near the visible arc, so center, side edge, and tip contacts all deal one 25-damage Ashwood hit without occupying future greatsword/axe scale.

**Implemented weapon-family foundation:** the Ashwood Blade selects the Balanced Slash presentation profile. Consecutive normal attacks cycle through a broad outward sweep, a reverse return sweep, and a farther-reaching visual finishing sweep, then repeat. This is visual variety rather than a damage combo: all three still use the weapon's same authoritative timing, hitbox, damage, and knockback. Swift Slash and Heavy Cleave profiles provide alternate orbit, extension, trail, strike-accent, and three-swing sequence tuning, but they are not obtainable weapons. A higher-grade sword may reuse Balanced Slash and Opaw's same weaponless body animation while changing its own texture, grip, scale, damage, knockback, and authoritative timing. A short sword, greatsword, axe, scythe, or other genuinely different family supplies its own reviewed contact shape and style through `WeaponDefinition`; visual arc or trail changes never change hitbox reach or damage by themselves.

**Implemented early weapon inventory:** Ashwood Blade is the always-owned, unsellable **Wood** fallback. Orren's **Iron Sword** costs 90 coins, deals 32 damage with 56 knockback, and reuses Balanced Slash plus Opaw's existing weaponless body animation. Stage I's full 46-coin reward cannot buy it; the implemented forest arc offers 144 coins total, making Iron a later earned milestone instead of an immediate purchase. Purchasing stores Iron without auto-equipping it. Orren's owned-item action and Character & Bag's detail panel both expose an explicit Equip button; selecting a slot only previews it. A valid equip synchronizes the authoritative `WeaponDefinition`, world sprite, menu portrait, damage, and knockback at the next safe idle boundary. Opaw's Warrior class rejects weapons whose `compatible_classes` omit `warrior`. Ownership and equipped choice autosave in Sanctuary. The current beginner set deliberately stops at Wood and Iron; spirit-blue higher grades remain deferred. Drops, armor stats, and selling are not implemented.

Equipment must reinforce decisions across normal attack, dash, and active skills rather than replace skills with larger numbers. The Forest's planned Stage V core-gear seal covers weapons, chest armor, gloves/bracers, and boots. Bracers are a gloves-family item rather than a second arm slot. Stages VI-VII introduce accessory components/blueprints, Stage VIII opens standard accessories, and Stage X opens relic/signature crafting. The paper doll retains two generic accessory positions that can accept pendant/amulet, earring, ring/charm, and later relic families instead of adding one slot per jewelry noun. Initial accessory stats may touch only implemented, capped authorities such as maximum vitality, regeneration, dash recovery, skill cooldown, or later resistance; mana, critical chance, lifesteal, status, and loot bonuses remain unavailable until their systems exist. Activating higher ranks also requires stronger authored enemies, elites, and bosses; Legendary or Mythic gear must not trivialize the beginner expeditions.
...
**Proposed King kit:** `Crescent Sever` becomes an aim-preview wedge/arc rather than a blind immediate-direction cast; `Riftfall Judgment` remains a movable ground-targeted collision-validated leap with impact AOE; `Sovereign Pursuit` remains actor-targeted; and `Worldsplitter: Last Horizon` uses a confirmed forward-line preview before cinematic commitment. Instant King skills, if added, must be self-AOE, aura, buff, defense, or naturally broad waves. All remain unimplemented until mouse/controller confirmation, cancel, range-clamp, contact, and invalid-target behavior pass temporary-shape tests.
...
Debug builds provide F9 to set the current run to level 10 and 999 coins, then swap in a separate loadout containing every currently authored Warrior skill: Piercing Rush and Consecutive Thrust. It also grants each authored Warrior-compatible weapon and deterministic sample quantities for otherwise-empty material IDs, so Ashwood/Iron scaling and every material filter/detail can be inspected without completing an expedition. This exists only to shorten testing, does not mark a normal awakening, Orren purchase, stage claim, or earned material reward as complete, does not expose unfinished slots 3-4, does not alter normal balance rules, and suppresses disk saving for that debug session.

The approved control budget is four equipable active skill slots on keys 1-4, in addition to normal attack and dash. One centered lower tray exposes dash followed by four fixed `52x48` icon-first controls; native GUI consumption prevents any HUD click from also becoming a basic attack. Normal play begins with Piercing Rush and may awaken Consecutive Thrust through Eira, while debug F9 remains a shortcut. The larger top-left vitality panel is prioritized over decoration, and a top-right `MENU [ESC]` button mirrors Escape. Physical Tab or the visible satchel/Character button opens the paused two-page character surface. `Character & Bag` places seven equipment positions around the live Opaw preview and uses a compact 24-slot grid for real owned weapons plus a capacity-free material pouch. All, Equipment, Materials, Consumables, and Key filters share one readable slot grammar; unimplemented categories remain empty instead of showing fake items. Selecting a weapon previews it and the explicit detail action equips it; selecting a material shows its saved quantity, family, source, and description without an equip action. `Active Skills` presents four compact `128x68` title/status selectors while the selected skill alone fills the description panel; Eira's service adds the free awakening action when eligible. Slots 3-4 remain sealed; their displayed level 6/9 milestones are presentation scaffolding until authored abilities and unlock rules are approved. Skills are never purchased.
...
Stages I-II have no clear chest: completing their final wave commits the collected expedition materials, opens the portal, and then records the stage-clear autosave. Stage III is the current milestone exception. Its larger Rootbound Reliquary arrives through a rune/spark reveal, participates in actor Y-sorting, and owns a small solid footprint plus an accessible interaction range. Pressing `F` grants a guaranteed ordinary-material payout from its first-clear/replay table, but no recipe or crafting-category unlock; the Husk's separate profile already guarantees its two boss materials. Claiming commits the expedition delta, releases the footprint, and opens the Sanctuary portal. Death, restart, or Return to Sanctuary before ordinary-stage clear or milestone-chest claim restores the expedition's starting material/recipe/claim state. Material quantities, miss counters, first-clear claims, and discovered blueprint IDs persist through the safe profile. Character & Bag displays the real icon, quantity, family, source, and description for earned stacks.

**Implemented Rootweaver preparation service:** Rootweaver Nema is the Sanctuary's attractive adult female grove smith, separate from Orren's ordinary weapon shop. Her west-mid open-air Living Rootforge, compact screen-left three-quarter idle/work actor, portrait dialogue, and paused menu let players inspect all four current recipes plus exact owned/required material quantities. The service is intentionally read-only: its disabled action explains the missing blueprint, Stage V core-gear or Stage VIII accessory seal, and unimplemented output equipment, and it never consumes materials or grants/discovers anything. Rootfiber Wraps and Huskbound Guard remain assigned to the planned Stage V core-gear milestone, while Mireward Charm and Thornward Clasp move to the planned Stage VIII standard-accessory unlock. Stages VI-VII prepare spirit/rune/binding components and Stage X remains the later relic/signature milestone. Leather begins only when a visible hide-bearing Stage 4 creature exists. Exact boss identities, seal/catalyst names, quantities, stats, and later recipes remain open content-contract work.
...
Open decisions include King's signature weapon and four skills, first crafted essence/relic stats and final recipe balance, the version-1 Opaw-save conversion policy, Hunt modifiers, selling policy beyond the current no-selling slice, and how defensive relics scale without erasing combat mastery.
