...
**Implemented Sanctuary loop:** Sanctuary is a safe expedition hub centered on a walk-around divine fountain and animated angel portal. King faces nearby speakers during interaction. Eira opens King's four-skill information surface without an awakening transaction; Orren is dialogue-only after retirement of the weapon shop; Nema opens the read-only Rootforge. The portal offers implemented stages and sealed future routes from immutable destination data. The active Forest expedition reaches Stage V and Varkuun.
...
- **Implemented controls:** WASD/left-stick movement owns King's four-way combat facing and standing preserves the last movement direction. Left click immediately performs a manual sword attack toward the nearest cardinal side of the clicked point. Right click is contextual: ground clears combat and creates a small two-frame four-arrow convergence mark, while an enemy selects beneath the overhead chevron and immediately auto-engages. Skills use `1`-`4`; Basic Attack has no number key and retains an icon-backed far-right HUD/right-trigger fallback. Click/assisted walking faces along actual navigation steering and WASD overrides pursuit. Targeted skills are modal: left click/right trigger confirms, while right click or `Esc` cancels without leaking a world command; the matching action slot pulses with `AIM RMB` during this state. `AUTO ALL` optionally cycles through live roster enemies; `AUTO SKILL` additionally uses ready authored skills at safe in-range boundaries.
...
This run state survives scene transitions and is captured at authored safe points: Sanctuary entry, equipment changes, and stage clears. Continue restores the last safe snapshot in Sanctuary; it does not resume an unfinished encounter. Reaching a level increases King's maximum health by 12 and produces the brief overhead level-up presentation. Story, materials, recipes, coins, level, XP, current health, and equipped gear persist through the versioned profile.
...
**Implemented Rootweaver preparation service:** Rootweaver Nema is the Sanctuary's grove smith; Orren is dialogue-only after retirement of the ordinary weapon shop. Her Living Rootforge presents six finalized Stage V recipes—Varkuun Edge, Old Bark Helm, Heartwood Plate, Rootfiber Gloves, Mirebound Leggings, and Mirehide Boots—with output icons, real stat copy, and owned/required materials. Two planned Stage VIII accessory recipes remain sealed previews. The service is read-only in normal play: it never consumes materials or grants equipment. F9 grants all outputs and satisfies blueprint/seal/material gates without saving.
