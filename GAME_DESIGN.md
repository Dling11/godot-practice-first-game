...
**Implemented Sanctuary loop:** Sanctuary is a safe expedition hub centered on a walk-around divine fountain and a fixed angel-gate structure whose doorway energy animates independently. King faces nearby speakers during interaction. Eira opens King's four-skill information surface without an awakening transaction; Orren is dialogue-only after retirement of the weapon shop; Nema opens the read-only Rootforge. The portal offers implemented stages and sealed future routes from immutable destination data. The active Forest expedition reaches Stage V and Varkuun.
