...
**Implemented Stage 1 encounter:** six sequential beginner waves introduce Mirelings, Rootlings, and then Thralls while never exceeding four live enemies. The later five-to-seven-enemy waves use brief, visibly announced reinforcements rather than an unreadable opening horde. Clearing the finale opens the stage-exit portal. Entering its range shows `F - Enter Portal`; leaving removes the prompt. Interaction fades through a loading veil into Stage 2.

**Implemented Stage 2 encounter — Thorns of the Forgotten Grove:** the player arrives at a small grove with a central broken shrine, flanking trees, and clear paths to the north exit. The arrival message reads `THE THORNS REMEMBER YOUR FOOTSTEPS`. Seven waves build from a three-Mireling warm-up through the first Bramble Spitter, Rootling/ranged pressure, and five-to-seven-enemy mixed endurance waves. Reinforcements keep the live crowd at four or fewer. Clearing all seven waves opens a return portal to Sanctuary; no exit portal exists before the clear.
...
**Rootbound Husk:** approved as the larger, later Stage 3 forest melee archetype but not yet integrated into a wave. This bark-and-vine humanoid anchors itself, telegraphs a longer crack in its locked forward direction, then erupts a Root Spear along that lane. It is a lateral-dodge lesson rather than a Thrall recolor, projectile user, or charge enemy. Its future Rootbound Elder boss may extend this clear base language with multi-lane roots and living-arena pressure.
