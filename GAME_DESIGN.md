...
**Umi and material exchange:** Umi is an unrelated Sanctuary blue witch whose Echo Crucible gives surplus materials two deliberate uses. Common materials may be sold for modest gold or consumed as meld fuel, but they are never reconstruction targets. Umi reconstructs known Uncommon, Rare, and protected Boss materials whose low drop rates can create a genuine drought. The target's own definition supplies value, costs, source enemy, and required victories, so later regional catalogs update the service without NPC-code branches. Uncommon/Rare reconstruction scales from 100/400 meld and 75/200 gold with 10/20 source defeats. Boss materials cannot be sold or used as fuel; reconstructing one costs 1500 meld, 1000 gold, four Rare materials from its region, the authored victory threshold, and one boss-memory charge earned every ten victories. Her intended campaign unlock is completion of Stage VIII; because Stage VIII has no runtime completion authority yet, Umi remains available in the current Sanctuary for production testing rather than using a false interim gate.
...
The final damage curve, player-facing poise bar, stamina, mana, and later targeting rules are not yet decided. Current crowd control is data-driven: Light enemies accept full knockback/stagger, Elite enemies accept reduced control, Heavy enemies accept 20% knockback and 30% stagger duration, and Boss enemies accept neither. A separate per-enemy chain limit may temporarily resist rapid repeat interruptions without suppressing damage or impact presentation.
...
**Implemented crowd-control baseline:** every enemy definition declares a `Light`, `Elite`, `Heavy`, or `Boss` control tier. Light enemies receive full movement knockback and any authored stagger; Elite enemies receive 35% knockback and 45% stagger duration; Heavy enemies receive 20% knockback and 30% stagger duration; Bosses receive no knockback or stagger. Stagger cancels a current enemy attack and temporarily blocks a new one, but only damage data that explicitly carries stagger can cause it. Thrall, Mireling, Rootling, and Bramble Spitter remain freely interruptible Light enemies. The Hog's third rapid interruption starts 0.8 seconds of stagger resistance; Crag Bear's fourth starts 1.05 seconds. The Bear's Heavy control profile does not make it an Elite or mini-boss. During resistance, health damage and all local impact feedback still resolve. Skill 1's authored stagger and Skill 3's longer 0.78-second base stagger pass through the same target scaling, so stronger techniques retain a stronger but bounded interruption. Accepted enemy stagger cancels King's vulnerable attack/cast and briefly recovers him while knockback remains Player-owned; invulnerability rejects the hit and explicit ability super armor preserves the cast.
...
**Bramble Spitter:** a 40-health Light ranged creature rebuilt around readable body motion. Four-direction idle, four-pose walk, hurt, collapse, and eight-pose spit families run through `AnimatedSprite2D`; the attack visibly notices, braces, inflates, compresses, thrusts, recoils, deflates, and recovers before VFX. Its approved art is rasterized at normal-ranged-mob scale inside the generous atlases: locomotion is roughly 28x25 opaque pixels, attack entry stays within 93-101% of the matching idle actor area, and the intentional mouth/thrust extension remains within 38x29. It never uses Hog/Bear-sized body pixels or permanent node scale. It seeks a 95-190 pixel firing band, may take one fixed 0.55-second/72-pixel retreat burst, then commits to firing when line of sight is available instead of kiting indefinitely. A red ground marker snapshots the player for 0.62 seconds. The 155 px/s animated thorn-seed deals 14 damage, terminates at the committed range/impact, and can also be destroyed by one accepted player hit through its non-selectable hurtbox. Its three-frame impact ends on a clean green/yellow burst with no spent-seed remnant. Recovery is 0.82 seconds. It is introduced in Stage 2 and returns as Stage VI ranged-priority pressure.
...
**Implemented Stage VI progression boundary:** Stage VI never checks for Varkuun Edge or armor before entry. Decision 127 removes Mirelings and uses approved returning roles only where they produce meaningful pressure: Crag Bear as the primary melee threat, Forsaken Thralls as screeners, Bramble Spitters as ranged priorities, and one finale Armored Hog. Varkuun Edge is intended to remain useful through roughly Stage XV. Exact crowd and material pacing remains tunable after starter-versus-crafted playtests.

**Implemented Stage VI — The Elder Ascent:** Decisions 124-127 preserve the restrained teal/pale-stone TileSet, continuous top-down elevated terrain, modular cliffs/trees/rocks, fixed-bank waterfall, and explicit collision/navigation. Five waves total 4/4/5/6/9 enemies under a five-live cap while preserving exactly eight Crag Bears. Fourteen Forsaken Thralls create horde screening, five rebuilt Bramble Spitters create ranged priority pressure, and one Armored Hog closes the finale; no Mireling appears. Crag Bear remains a normal mob with the approved eight-pose claw/body-slam families, dedicated action audio, hitbox-sized slam cracks/dust/stone, and restrained camera pulse. Difficulty is composition-first rather than Bear HP/damage inflation. Clear banks loot, records Stage VI completion/discovery, and opens a Normal portal to Sanctuary.

**Provisional Stage VII story hook:** a god-tier follower may appear as an optional confrontation after brief dialogue. The player should retain an honest choice to continue or challenge it; if challenged, the scene should demonstrate the power gap through a readable survival or recognition trial rather than a cheap unavoidable scripted death. Identity, dialogue, reward, combat rules, and whether this becomes the final Stage VII structure remain undecided.
...
**Implemented Forest reward acquisition:** the stable current material set is Mire Resin/Mire Membrane from Mirelings, Root Fiber/Young Heartwood from Rootlings, Forsaken Cloth/Weathered Fittings from Thralls, Barbed Seed/Thorn Sap from Bramble Spitters, Husk Heartwood/Rootbound Core from the Rootbound Husk, and Crag Iron/Echo Claw from Crag Bears. Each has a distinct 24x24 icon. Crag Iron rolls at 30% with fifth-attempt protection; Echo Claw rolls at 8% with twelfth-attempt protection, targeting roughly two to three Iron and zero to one Claw across Stage VI's eight Bears. Final accessory quantities remain a Stage VIII recipe decision. Enemy deaths resolve entries through one reward authority; resolved stacks hop, hover, then fly to King. The Rootbound Husk always supplies both boss materials.

Stages I-II have no clear chest: completing their final wave commits the collected expedition materials, opens the portal, and then records the stage-clear autosave. Stage III's mini-boss Rootbound Reliquary grants guaranteed ordinary pooled materials but no recipe or crafting-category unlock. Stage V's major-boss Varkuun's Chest uses a larger dead-root/iron silhouette and spawns where he falls. First clear grants two Varkuun Cores plus the permanent Forest core-gear seal/discovery; replay grants one Varkuun Core and never repeats the seal. Claiming commits the expedition delta, releases the footprint, records completion, and opens the blue Normal portal directly into Stage VI. Death, restart, or Return to Sanctuary before ordinary-stage clear or milestone-chest claim restores the expedition's starting material/recipe/claim state.
