...
**Implemented Forest reward acquisition:** the stable current material set is Mire Resin/Mire Membrane from Mirelings, Root Fiber/Young Heartwood from Rootlings, Forsaken Cloth/Weathered Fittings from Thralls, Barbed Seed/Thorn Sap from Bramble Spitters, and Husk Heartwood/Rootbound Core from the Rootbound Husk. Each has a distinct 24x24 icon. Ordinary common materials are no longer universal rewards: Mire Resin and Root Fiber roll at 45%, Forsaken Cloth at 50%, and Barbed Seed at 55%, with the third consecutive unresolved common attempt forced. Secondary materials use lower 20-28% rolls and a fifth-attempt protection cap. Enemy deaths resolve those common, optional, rare, and boss entries through one reward authority. A resolved stack visibly hops and hovers at the death point for 0.5 seconds before flying automatically to Opaw; early contact also collects it and a compact HUD toast confirms the grant. The Rootbound Husk always supplies both Husk Heartwood and Rootbound Core.

Every completed Stage I-III encounter now spawns a non-empty reward chest in place of the portal. It arrives through a short rune/spark reveal, participates in actor Y-sorting, owns a small solid footprint, and keeps its interaction area outside that footprint so Opaw can approach without overlapping it. Stages I-II use the ordinary Forest Cache; Stage III uses the larger Rootbound Reliquary mini-boss tier. The player approaches and presses `F`; first clear grants its authored materials and blueprint/discovery set exactly once, while later claims use the replay table. Chest rarity changes visual language and interaction emphasis, not reward authority. Claiming commits the expedition reward delta, releases the chest collision, and opens the portal. Death, restart, or Return to Sanctuary before the chest claim restores the expedition's starting material/recipe/claim state. Material quantities, miss counters, first-clear claims, and discovered blueprint IDs persist through the safe profile. Character & Bag displays the real icon, quantity, family, source, and description for earned stacks.

**Approved next Forest economy behavior:** a separate Rootweaver/Grove Artificer service will own crafting presentation and deterministic craft requests; Orren remains the weapon merchant. Rootfiber Wraps, Mireward Charm, Thornward Clasp, and Huskbound Guard are discoverable blueprints but do not yet resolve to craftable/equippable outputs. Leather begins only when a visible hide-bearing Stage 4 creature exists. Initial reward quantities are implementation tuning and may change after playtesting, but drops must continue to match each creature's body, habitat, and combat behavior.
...
Open decisions include which abilities occupy slots 3-4, first crafted equipment stats and final recipe balance, the craftsperson's final name/design, Hunt modifiers, selling policy beyond the current no-selling slice, and how armor bonuses scale without erasing combat mastery.
