...
**Stage VI environment foundation:** Decisions 124-125 establish the working route `The Elder Ascent` before enemy production. Its approved restrained teal/pale-stone TileSet remains the visual baseline. The upper map continues that same top-down world into elevated terrain bounded by modular grass-topped cliffs, existing top-down ancient trees, reusable rocks, and a simple four-frame waterfall descending toward the broad lower arena. Sky, horizon, distant side-view landscape, and one-use scenic backgrounds are excluded. Explicit cliff collision remains independent from art. The current scene is an environment preview only and does not yet advance the campaign.
