...
This is in-memory run state: it survives portal scene transitions, resets when the player restarts after defeat, and is not written to disk. Reaching a level does not pause combat or randomly offer upgrades; it increases Opaw's maximum health by 12 and produces the brief overhead level-up presentation. Level 3 creates Skill 2 eligibility; Eira's separate free action records the awakening in session-only story memory.
