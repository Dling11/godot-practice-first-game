...
**King animation/targeting direction:** Decision 077 makes the Decision 076 simple King the temporary active combat proof. Four-direction idle/walk and one broad four-phase basic slash are implemented; the current normal attack still uses the proven shared Balanced Slash timing/contact authority while King's real tap/hold chain remains future work. Locomotion-derived dash/reaction aliases are safe placeholders, not final action art. Decision 078 implements Echoing Sever with continuous 360-degree wedge aim; its hitbox and VFX keep the exact angle while the body selects the nearest existing cardinal animation. The other three King skill slots remain visibly sealed.
