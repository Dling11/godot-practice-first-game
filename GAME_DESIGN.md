...
- **Implemented input commitment:** One latest valid normal attack or immediate-directional skill may buffer through a committed normal attack or active dash and starts at the first vulnerable recovery boundary. A living hit window is never interrupted and inputs do not stack. Skills remain committed unless their data explicitly permits dash cancellation; Consecutive Thrust is the first such exception.
...
- **Armskeeper Orren:** implemented Sanctuary merchant NPC who sells the Warrior-only Iron Sword for 90 coins and never sells skills.
...
Future weapon families may include spears, dual swords, greatswords, axes, scythes, ranged weapons, and supernatural weapons. Each melee family should author an appropriate contact shape, grip, arc, timing, and presentation through the shared contracts rather than inheriting Balanced Slash or merely reskinning the sword. Pets remain a separate future companion-system decision.
...
This run state survives scene transitions and is captured at authored safe points: Sanctuary entry, Sanctuary purchase/equip/awakening actions, and stage clears. Continue restores the last safe snapshot in Sanctuary; it does not resume an unfinished encounter. Reaching a level does not pause combat or randomly offer upgrades; it increases Opaw's maximum health by 12 and produces the brief overhead level-up presentation. Level 3 creates Skill 2 eligibility; Eira's separate free action records the awakening in persisted story memory.
...
**Implemented Rootweaver preparation service:** Rootweaver Nema is the Sanctuary's attractive adult female grove smith, separate from Orren's ordinary weapon shop. Her southwest open-air Living Rootforge, compact idle/work actor, portrait dialogue, and paused menu let players inspect all four current recipes plus exact owned/required material quantities. The service is intentionally read-only: its disabled action explains the missing blueprint, Stage V or Stage X seal, and unimplemented output equipment, and it never consumes materials or grants/discovers anything. Rootfiber Wraps and Huskbound Guard remain assigned to the planned Stage V weapon/armor milestone, while Mireward Charm and Thornward Clasp remain assigned to the planned Stage X accessory/relic unlock. Stage VIII is a second mini-boss material payout focused on fungal, spirit, binding, and relic-setting preparation; it does not replace Stage X's permanent unlock. Leather begins only when a visible hide-bearing Stage 4 creature exists. Exact boss identities, seal/catalyst names, quantities, stats, and later recipes remain open content-contract work.
...
Open decisions include which abilities occupy slots 3-4, first crafted equipment stats and final recipe balance, Hunt modifiers, selling policy beyond the current no-selling slice, and how armor bonuses scale without erasing combat mastery.
