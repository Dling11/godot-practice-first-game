...
const EARTH_OUT := "res://art_source/review/characters/king/earthsplitter_pressure_2026_09_14/"
func _run() -> void:
	...
	caption.text = "EARTHSPLITTER / FIXED REACH / DIRECTIONAL PRESSURE"
	await _wait(.4)
	for direction in [Vector2.RIGHT, Vector2(1,1).normalized(), Vector2.DOWN, Vector2(-1,1).normalized(), Vector2.LEFT, Vector2(-1,-1).normalized(), Vector2.UP, Vector2(1,-1).normalized(), Vector2.RIGHT.rotated(deg_to_rad(17))]:
		...
		caption.text = "CONTROL RETURNS / PRESSURE CONTINUES INDEPENDENTLY"
		await _wait(.13)
