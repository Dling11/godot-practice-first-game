...
const OUT := "res://art_source/review/characters/disciples/examiner/polish_2026_09_12/"
var lab: Node
...
func _run() -> void:
	...
	for direction: Vector2 in [Vector2.RIGHT, Vector2.LEFT, Vector2.DOWN, Vector2.UP]:
		examiner.global_position = Vector2(365, 285) - direction * 65
	...
	_drive = true
	examiner.global_position = Vector2(400, 275)
	lab.player.global_position = Vector2(280, 275)
	examiner._begin_combo(Vector2.LEFT)
	await create_timer(.46).timeout
	await _capture("02_left_thrust")
	...
	await _capture("12b_descent_peak")
