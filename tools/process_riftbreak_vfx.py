"""Normalize Riftbreak's generated 3x2 VFX board for Godot runtime use."""
...
SOURCE = ROOT / "art_source/cleaned/characters/playable/king/simple_reboot/riftbreak/riftbreak_vfx_clean_v1.png"
RUNTIME_DIR = ROOT / "assets/vfx/abilities/king"
REVIEW_DIR = ROOT / "art_source/review/characters/playable/king/simple_reboot/riftbreak"
SHEET = RUNTIME_DIR / "riftbreak_vfx_sheet_192.png"
FRAMES = RUNTIME_DIR / "riftbreak_vfx_frames.tres"
REVIEW = REVIEW_DIR / "riftbreak_vfx_review_4x.png"
...
def _atlas_resource(frame_index: int) -> str:
    ...
    return "\n".join(
        [
            f'[sub_resource type="AtlasTexture" id="RiftbreakAtlas_{frame_index}"]',
...
def _animation(name: str, frame_indices: list[int], speed: float, loop: bool) -> str:
    entries = ", ".join(
        f'{{"duration": 1.0, "texture": SubResource("RiftbreakAtlas_{index}")}}'
...
def main() -> None:
    ...
    if source.size != expected:
        raise RuntimeError(f"Riftbreak source must be {expected}, got {source.size}")
    ...
    for frame_index in range(FRAME_COUNT):
        ...
        ).resize((RUNTIME_CELL, RUNTIME_CELL), Image.Resampling.NEAREST)
        binary_alpha = cell.getchannel("A").point(
            lambda value: 255 if value >= ALPHA_THRESHOLD else 0
        )
        bbox = binary_alpha.getbbox()
        if bbox is None:
            raise RuntimeError(f"Riftbreak frame {frame_index} is empty")
    ...
    review.alpha_composite(
        atlas.resize((atlas.width * 4, atlas.height * 4), Image.Resampling.NEAREST),
        (0, 0),
    )
    ...
    lines = [
        ...
        "",
        '[ext_resource type="Texture2D" path="res://assets/vfx/abilities/king/riftbreak_vfx_sheet_192.png" id="1_vfx"]',
        "",
    ]
    for frame_index in range(FRAME_COUNT):
        lines.append(_atlas_resource(frame_index))
    animations = [
        _animation("wind_up", [0, 1], 12.0, False),
