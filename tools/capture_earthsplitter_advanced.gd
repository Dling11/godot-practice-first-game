...
const EARTH_OUT := "res://art_source/review/characters/king/earthsplitter_advanced_2026_09_14/"
var _capture_prefix := "foundation_"
func _run() -> void:
	...
	if "--ui-proof" in OS.get_cmdline_user_args():
		review.toggle_earthsplitter_form()
		await _wait(.2)
		await RenderingServer.frame_post_draw
		root.get_texture().get_image().save_png(EARTH_OUT + "playable_lab.png")
		lab.queue_free()
		await process_frame
		await process_frame
	...
	lab.camera.zoom = Vector2(1.4,1.4)
	...
	caption.text = "EARTHSPLITTER / FOUNDATION AND ADVANCED"
	await _wait(.4)
	var cast_index := 0
	for direction in [Vector2.RIGHT, Vector2.RIGHT, Vector2(1,1).normalized(), Vector2.DOWN, Vector2(-1,1).normalized(), Vector2.LEFT, Vector2(-1,-1).normalized(), Vector2.UP, Vector2(1,-1).normalized(), Vector2.RIGHT.rotated(deg_to_rad(17))]:
		if cast_index == 1:
			review.toggle_earthsplitter_form()
			_capture_prefix = "advanced_"
		caption.text = "FOUNDATION / APPROVED SINGLE WAVE" if cast_index==0 else "ADVANCED / THREE WAVES / ONE QUICK CAST"
		cast_index += 1
		actor.position = Vector2(425,310) - direction*90
		...
		await _wait(.10)
		await _shot("finisher_" + str(direction))
		await _wait(.55)
	...
	for x in [415,555,590]:
		...
		foes.append(foe)
	await _wait(.4)
	caption.text = "LIVE DAMAGE / SHARED THREE-WAVE BUDGET / NO STUN"
	actor.ability_1_component.clear_cooldown()
...
func _shot(label: String) -> void:
	await RenderingServer.frame_post_draw
	root.get_texture().get_image().save_png(EARTH_OUT + _capture_prefix + label.replace(" ","").replace(",","_").replace("(","").replace(")","") + ".png")
