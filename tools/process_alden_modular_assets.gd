...
## Converts Alden's separately authored action sources and Ashwood Blade source
## into deterministic, binary-alpha runtime art. Gameplay scenes never load the
## generation sources directly.

const CHARACTER_SOURCE_ROOT := "res://art_source/generated/characters/playable/alden/v2"
const CHARACTER_OUTPUT_ROOT := "res://assets/characters/playable/alden"
...
const CHARACTER_ACTIONS := [
	{
		"name": "idle",
		"source": "alden_idle_source.png",
		"clean": "alden_idle_clean.png",
		"output": "alden_idle_sheet_32x32.png",
		"columns": 2,
		"cell_size": CHARACTER_CELL_SIZE,
	},
	{
		"name": "walk",
		"source": "alden_walk_source.png",
		"clean": "alden_walk_clean.png",
		"output": "alden_walk_sheet_32x32.png",
		"columns": 4,
		"cell_size": CHARACTER_CELL_SIZE,
	},
	{
		"name": "attack_body",
		"source": "alden_attack_body_source.png",
		"clean": "alden_attack_body_clean.png",
		"output": "alden_attack_body_sheet_48x32.png",
		"columns": 3,
		"cell_size": CHARACTER_ACTION_CELL_SIZE,
	},
	{
		"name": "dash",
		"source": "alden_dash_source.png",
		"clean": "alden_dash_clean.png",
		"output": "alden_dash_sheet_48x32.png",
		"columns": 3,
		"cell_size": CHARACTER_ACTION_CELL_SIZE,
	},
	{
		"name": "interact",
		"source": "alden_interact_source.png",
		"clean": "alden_interact_clean.png",
		"output": "alden_interact_sheet_48x32.png",
		"columns": 2,
		"cell_size": CHARACTER_ACTION_CELL_SIZE,
	},
	{
		"name": "hurt",
		"source": "alden_hurt_source.png",
		"clean": "alden_hurt_clean.png",
		"output": "alden_hurt_sheet_32x32.png",
		"columns": 2,
		"cell_size": CHARACTER_CELL_SIZE,
	},
	{
		"name": "defeat",
		"source": "alden_defeat_source.png",
		"clean": "alden_defeat_clean.png",
		"output": "alden_defeat_sheet_64x32.png",
		"columns": 4,
...
func _initialize() -> void:
	if not _process_character_actions():
		quit(1)
		return
	if not _process_weapon():
		quit(1)
		return
	print("Processed Alden's separate action sheets and Ashwood Blade runtime art.")
	quit(0)
...
func _process_character_action(action: Dictionary) -> bool:
	...
	if source == null or source.is_empty():
		return _fail("Unable to load Alden %s source: %s" % [action.name, source_path])
	...
	if clean_error != OK:
		return _fail("Unable to save Alden %s clean source: %s" % [action.name, error_string(clean_error)])
	...
	for row in CHARACTER_DIRECTION_ROWS:
		...
		if reference_bounds.size.x <= 0 or reference_bounds.size.y <= 0:
			return _fail("Alden %s row %s has an empty reference frame." % [action.name, row])
		...
		for column in columns:
			...
			if normalized == null or normalized.is_empty():
				return _fail("Alden %s source frame %s,%s is empty." % [action.name, column, row])
	...
	if output_error != OK:
		return _fail("Unable to save Alden %s runtime sheet: %s" % [action.name, error_string(output_error)])
	return true
