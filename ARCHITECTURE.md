...
`EncounterController` replaces final-wave portal creation with one configured `StageRewardChest`. It adds the chest to the same Y-sorted `Actors` owner as the player and props, while the portal and non-spatial effects retain their existing parents. The chest owns a small solid ground footprint, a larger interaction area, and a presentation-only reusable rune/spark reveal. It submits its stage `LootTableDefinition` to `LootService` only after explicit `F` interaction, changes to its tier-specific open art on success, releases its physical footprint, and reports the claim to the controller. Stages I-II select the ordinary Forest Cache tier; Stage III selects the Rootbound Reliquary tier. Tier selection owns art, prompt copy, footprint size, and accent only—it never changes reward resolution. The controller then creates the ordinary portal and emits `stage_cleared`, preserving the existing story record and autosave order. First-clear versus replay selection comes solely from the table's stable claim ID in `LootState`; the chest cannot duplicate a one-time reward by reopening or reloading.

`LootService.begin_expedition()` snapshots material quantities, recipe discoveries, first-clear claims, and bad-luck counters at stage entry. Chest success commits that baseline. Defeat restart, pause-menu Return to Sanctuary, and defensive Sanctuary entry restore it first, so unclaimed combat drops never leak through an abandoned expedition or get written by the next safe checkpoint. XP, coins, and current HP retain their existing run semantics. Segment 5 must add `CraftingService` and actual output/equipment integration before any discovered recipe is described as craftable.

The planned Sanctuary Rootweaver/Grove Artificer is a presentation/service boundary separate from Armskeeper Orren. Its dialogue and craft menu observe recipe/inventory state and submit commands; it does not contain recipe balances in scene nodes. Stage-clear claims are already idempotent at the save boundary, while future explicitly replayable Hunt rewards must remain repeatable only by authored rule.
