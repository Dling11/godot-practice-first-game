...
- `CombatTargetMarker`, `ClickMoveMarker`, `CombatFootAura`, and `CombatHUD` observe combat state for a restrained overhead selection chevron, small two-frame four-arrow destination mark, tier-aware presentation-only actor footprint auras, left target card, and compact top-right live enemy roster. Right-click world intent routes through `Player`: an enemy hit calls `select_at_world_position(..., true)` for immediate assisted engagement, while empty ground calls `request_click_move()` and clears combat. `PlayerAutoCombatComponent` optionally cycles through the roster and requests ready skills through the same public Player/targeting authority; it never deals damage or changes cooldowns. The unnumbered Attack HUD fallback calls `Player.request_assisted_primary_attack()`. `Player` resolves click/assisted facing from navigation steering, direct manual attacks from the clicked world direction, and direct target facing only while stationary/in range.
...
`PlayerInputSource` exposes movement, right-stick aim, and discrete action intent. Ordinary combat still never derives facing from passive pointer motion: `Player` resolves movement to one retained cardinal facing. Outside targeting, left click requests the immediate manual sword attack and right click resolves the contextual world command. `DirectionalWedgeTargeting` becomes active only for a compatible requested skill, owns exact 360-degree preview direction/geometry but no cost or damage, and gives right-stick input priority over the pointer. Movement preserves that state; dash cancels it before starting. Left-click/right-trigger confirms instead of leaking into a normal attack, repeating the skill input is consumed without commitment, and right click or `Esc` cancels. Modal targeting consumes either mouse command before world movement/engagement can resolve. Only the emitted exact confirmed direction reaches `AbilityComponent.request_cast()`, where cooldown and damage authority begin together; the body independently resolves that direction to its nearest available cardinal animation.
...
The lower HUD and `CharacterMenu` consume the same player-owned `SkillLoadoutDefinition`. `CombatHUD` creates four icon-first `SkillBarSlot` observers plus one `DashBarSlot`; each native click button forwards intent to `Player`, while ability/evade components own readiness and emit cooldown state. Fixed-size controls show only icon, key, short state, and cooldown bar so localized names cannot alter layout. The HUD observes directional/ground targeting signals only to pulse the matching corner-accented slot and show `AIM RMB`; it never confirms, cancels, or spends a skill itself. Cooldown slots remain clickable so the shared denied-action cue can respond, while unbound definitions remain visibly sealed. The top-right `MENU [ESC]` button locates the current scene's registered `PauseMenu`; it does not own pause state. `CharacterMenu` creates four fixed `128x68` `SkillSlotCard` title/status buttons and places the full selected description in one detail strip, preventing repeated copy from expanding the modal. Ground-target definitions are rejected until a separate targeting authority supplies a confirmed point.

King's live skill and hardware-cursor presentation uses generated raster sources with deterministic runtime derivatives. Approved B+C skill sources live in `art_source/generated/ui/king_skill_icons_bc_hybrid/`; `tools/prepare_generated_combat_ui.gd` chroma-cleans where required, normalizes to the 48-pixel logical grid, maps to King's fixed 14-color palette, reduces exactly 2:1 with nearest filtering, and emits binary-alpha 24x24 skill PNGs under `assets/ui/icons/skills/generated/`. The separate cursor experiment retains its source under `art_source/generated/ui/king_combat_ui/` and 32-pixel derivatives under `assets/ui/cursors/generated/`. Ability definitions and `CursorService` reference only runtime derivatives. The pipeline changes presentation files only and never participates in cast, movement, damage, targeting, automation, or input decisions.
