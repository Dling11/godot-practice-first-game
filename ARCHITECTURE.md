...
`RootboundHusk` is a separate Boss-tier `CharacterBody2D` controller. It directly preloads its attack-profile script so editor parsing does not depend on newly refreshed global-class metadata. The immutable profile owns separate spear/fan timings, fan spread and side delay, plus below-half-health timing/spread values; `EnemyDefinition` still owns health, damage, range, movement, and rewards. The controller chooses a quick single Root Spear or slower Root Fan, snapshots every direction, rotates the authoritative 112x20 hitboxes before wind-up, activates the fan's center and side lanes in separate windows, and emits one `root_attack_started` event per full attack for audio. `RootboundHuskVisual` observes facing, movement, state, damage, telegraph, and staged eruption signals through one `AnimatedSprite2D` body and three ground-layer `AnimatedSprite2D` VFX nodes; pending side warnings remain visible after the center eruption. VFX placement uses the lane center on the ground plane with no directional sprite rotation, so the radial crack/root flow cannot appear to originate from a hand. Named `SpriteFrames` animations own playback only and never activate damage. `tools/assemble_rootbound_husk_redesign.gd` reproducibly composes both v4 body sources: it builds walk from approved down/up direction rows plus individually reviewed side contact/passing poses, uses the complete side-strip contact rather than the fragment-prone direction-board pose, recovers each up-facing actor through an 80-pixel bounded overlap, and derives left from the approved right cycle; it also recovers every actor from the raw v4 6x4 root-attack board through a 64-pixel bounded overlap, substitutes a dedicated two-frame front-facing down-active strip, and normalizes one shared scale per direction row into an exact-grid source. `tools/process_rootbound_husk_assets.gd` chroma-cleans assembled v4 walk/root-attack plus v3 reaction sources, retains complete body components while removing detached debris, applies one standing-reference scale and foot baseline per direction row, emits `72x64` walk, `96x64` root-attack, and `64x64` reaction sheets, then overwrites every normalized right side row with a byte-verified mirror of left. It separately preserves the existing six-stage `128x64` ground-root VFX. `tools/build_rootbound_husk_sprite_frames.gd` creates the exact 28 body animations plus two VFX animations after texture import, and `tests/rootbound_husk_smoke.gd` rejects stale animation groups, clipped crown silhouettes, side-turning down-active frames, wrong root-attack atlas mappings, or side walk frames outside the 56-pixel visible-height contract. Root-command presentation is named only `root_attack_wind_up_*`, `root_attack_active_*`, and `root_attack_recovery_*`; every cast-named asset, animation, import cache, and retired Husk body archive was permanently deleted.

`EncounterController` owns a stage's data-driven wave lifecycle, bounded reinforcement queue, injects the shared `World/Projectiles` parent into projectile-capable enemies, and creates one `StagePortal` after the final wave. `EncounterWaveDefinition` owns counts, initial cadence, and reinforcement delay; the controller never allows more than its configured four active enemies. Each queued replacement emits `reinforcement_announced`, waits the authored delay, then releases one enemy so burst kills earn a readable reset instead of instantly backfilling the crowd. HUD observes that signal for its warning. `StagePortal` owns player proximity and F-input, emits prompt visibility, and delegates valid destinations to the `SceneTransition` autoload. HUD owns prompt presentation. Stage 1 has six beginner waves and targets Stage 2; Stage 2 has seven authored Grove waves and opens its return portal only after clear.
