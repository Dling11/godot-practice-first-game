...
Each current enemy scene configures its existing `EnemyRewardComponent` with one `DropProfileDefinition`. On authoritative death, that component requests a resolved result from `LootService` and instantiates one `MaterialPickup` per combined stack under the stage's Y-sorted actor owner. The pickup owns only icon/pop/contact/fade presentation; walking over it asks `LootService` to grant the already-resolved stack. Physics monitoring changes from an overlap callback are deferred. The HUD observes `material_granted` for a compact toast and never calculates a drop.

`EncounterController` replaces final-wave portal creation with one configured `StageRewardChest`. The chest submits its stage `LootTableDefinition` to `LootService` only after explicit `F` interaction, changes to its open art on success, and reports the claim to the controller. The controller then creates the ordinary portal and emits `stage_cleared`, preserving the existing story record and autosave order. First-clear versus replay selection comes solely from the table's stable claim ID in `LootState`; the chest cannot duplicate a one-time reward by reopening or reloading.
