...
`ExaminerTrial` owns accepted-damage accumulation, physics-timed warned cuts, timeout, and cancellation. The actor owns combo branching, committed physical travel, phase entry, and sanctuary choice. Court owns the active sanctuary list, exact protection geometry, shader flags, and Verdict damage. Director owns dialogue/camera/music and releases cinematic input before the trial. `DamageInfo.ignores_invulnerability` is opt-in; `HealthComponent.is_damage_immune` preserves explicit debug immunity separately from temporary action i-frames. Other damage sources retain existing behavior.
