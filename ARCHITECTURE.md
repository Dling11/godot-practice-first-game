...
`GroundPointTargeting` owns only Skill 3 intent/preview. `SovereignPursuitComponent` clamps the requested point, exposes traversal velocity to `Player` movement authority, grants protection only during its active phase, and opens one radial hitbox window at the actual collision-safe landing position. `SovereignPursuitTravelVisual` is a separate character-local observer that shows the open-center power sheath only during traversal. `SovereignPursuitVisual` stays top-level so launch dust remains at takeoff, then relocates only on the authoritative strike, plays a presentation-only landing shockwave, and independently fades its crater after recovery. `RiftbreakVisual` likewise captures one world-space cast origin before its residual lifetime begins, so neither ground mark inherits later player movement. Body, VFX, cursor, and audio never calculate damage or relocate the actor.
...
`Player` owns one non-stacking latest-intent combat buffer backed by a one-shot 0.8-second `Timer`. It stores normal attack, dash, or one equipped-skill slot plus captured direction. A later valid intent replaces the first; expiry, defeat, or execution clears it. Attack/dash recovery signals and `ability_finished` are the only automatic execution boundaries. Immediate/self-area skills start there, targeted skills reopen their preview, and inputs unavailable on cooldown are rejected rather than retained. Existing authored cancellation remains separate: a basic attack may replace vulnerable dash recovery, and only opt-in skills may be canceled by dash. Active hit and invulnerability windows never overlap with the buffered follow-up.
