...
`Stage5Boss` is presently an isolated vertical-slice controller, not stage authority. It owns the lunge/slap hitboxes, target-locked jump, and a fourth post-jump root-execution state family. `Stage5BossRootPrison` is world presentation only: it tracks during warning, then remains fixed and observes player restraint progress to choose flexing intact/cracked/broken frames before the execution atlas plays. A nearby world Label and `CombatHUD` both observe the same progress; `DashBarSlot` becomes an enabled `TAP / BREAK n/5` control during restraint and restores its normal cooldown grammar afterward. `Player` owns the reusable authoritative restraint seam. Capture clears buffered actions and targeting, cancels active attack/evade/abilities, freezes locomotion, rejects attack/skill requests, and converts `request_evade()`—including the HUD/mobile dash button—into one break point. Final-zero release validates source ownership independently of remaining points so the fifth input always emits `restraint_ended`. The execution processor groups complete components around eight source anchors rather than crop-first strips. The boss checks restraint ownership before applying its controller-owned 300 damage and releases on execution/death. `Stage5BossVisual` still exposes 56 directional animations; root states reuse and hold the planted slap pose. Focused smoke tests protect all four actions, VFX scale/margins, prompts, world locks, and escape/failure branches.

`DebugAdminState` is a session-only autoload that accepts enablement only in debug builds and never enters save/profile data. Sanctuary observes its signal to reveal the Combat Lab panel; F7/F8 routing is rejected while Admin Mode is disabled. `CombatLab` composes the real player, navigation, projectiles, effects, combat feedback, and canonical enemy scenes. Before an instantiated enemy enters the scene tree, the lab removes `EnemyRewardComponent`, injects the real player or `null` as its shared target for AI control, and supplies a lab-owned projectile parent. Clear/reset owns only lab children. Player debug combat tools expose the authored test loadout and cooldown bypass without applying F9's progression/material/story grants. Release builds refuse the lab scene and route back to Sanctuary.
