...
Disk persistence is not implemented. `SaveService` is now the narrow profile-schema coordinator: it composes version-1 snapshots from `RunSession`, `StoryState`, and `WeaponInventory`, validates every nested version before mutation, restores the three authorities only after complete validation, and records a safe scene path. Its `extensions` dictionary reserves stable seams for future material inventory, recipe discovery, stage claims, and regional progress without claiming those systems exist. `RunSession` and `WeaponInventory` now own their explicit versioned snapshot/restore contracts alongside the existing `StoryState` contract.

`SaveService` deliberately performs no file I/O yet. Segment 1 still requires JSON serialization, temporary-file replacement, backup/corruption recovery, migrations, safe-milestone autosave, and title-screen Continue. Settings remain separate from profile progression. Do not serialize arbitrary scene trees or mid-attack state as the save format.

### Planned Loot and Crafting Boundary

The approved Forest loot/crafting architecture is specified in `docs/design/forest-loot-crafting-and-regional-material-plan.md`; none of it is runtime code yet. Immutable `MaterialDefinition`, `DropProfileDefinition`, `LootTableDefinition`, and `RecipeDefinition` resources should own stable IDs, presentation references, roll/guarantee rules, ingredient/output requirements, and unlock metadata. Planned paths are `data/items/materials/`, `data/loot/drop_profiles/`, `data/loot/tables/`, and `data/crafting/recipes/`.

Mutable material counts belong to a dedicated `MaterialInventory`; crafting validation and atomic ingredient consumption/output grant belong to `CraftingService`; disk snapshots and migrations belong to `SaveService`. Existing `RunSession`, `WeaponInventory`, and `StoryState` remain authorities for their current domains. Enemy controllers may report a defeated enemy/drop-profile identity, encounter flow may request the authored clear chest, and UI may preview/request actions, but none of them may independently roll loot, mutate material counts, evaluate recipes, or write save files.
