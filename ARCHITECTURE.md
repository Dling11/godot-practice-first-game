...
- `CombatTargetMarker`, `ClickMoveMarker`, and `CombatHUD` observe the targeting component for a red footprint-sized selected-target circle, blue click-move mark, left target card, and compact live enemy roster. The dedicated BASIC ATTACK button calls back into `Player.request_assisted_primary_attack()` and owns no selection, movement, or damage authority.
