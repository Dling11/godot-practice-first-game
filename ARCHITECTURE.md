...
`res://tests/sanctuary_hub_smoke.gd` verifies the dedicated tileset, eleven normalized binary-alpha assets, NPC idle animation, Eira-to-skill-menu handoff, Orren dialogue, split portal depth/idle layers, the compact doorway trigger, continuous six-pixel-footprint routes around both fountain sides, prompt ownership, pause restoration, and the Stage 1 destination.
