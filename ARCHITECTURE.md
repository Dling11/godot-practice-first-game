...
`res://tests/character_animation_smoke.gd` verifies all seven binary-alpha Alden action sheets, every populated cell, 18x27 directional reference scale, shared foot baseline, separate three-frame attack/dash data, attack-phase frame mapping, unchanged node scale, definition-owned grip motion, hurt recovery, four-frame defeat/fade, plus the existing Thrall action-art and foot-plane ownership contracts.
