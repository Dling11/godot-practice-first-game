...
`SaveService` owns one versioned disk-profile boundary. It composes version-1 snapshots from `RunSession`, `StoryState`, and `WeaponInventory`, validates every nested version before mutation, restores the three authorities only after complete validation, and records Sanctuary as the current safe scene. Its `extensions` dictionary reserves stable seams for future material inventory, recipe discovery, stage claims, and regional progress without claiming those systems exist. `RunSession`, `StoryState`, and `WeaponInventory` own their explicit versioned snapshot/restore contracts; none performs file I/O.
