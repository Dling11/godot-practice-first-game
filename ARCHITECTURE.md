...
- A future Stage VII flow must own the Continue/Challenge decision, safe-point boundary, expected-loss interception, recognition threshold, abnormal-victory outcome, and onward transition. The Examiner controller owns combat behavior only; `DialoguePanel`, portal presentation, Split Glaive secondary layers, VFX, and health UI cannot decide the story result.
- The current Examiner implementation is deliberately contained by `levels/combat_lab/`: `Examiner` owns combat decisions, state timing, hitboxes, Axiom checks, velocity-derived locomotion facing, and the physical Divine Descent state machine; `examiner_visual.gd` maps those states to nine independently replaceable `AnimatedSprite2D` sheets and owns only the 0.055-second body silhouette/red-eye accent. `CourtOfFirstMeasure` owns the four exact protection zones, charging seal, and finite Descent damage. `ExaminerEncounterDirector` coordinates only player cinematic lock, camera, dialogue, a single cancellable music foundation, and the hand-off between actor and arena; rebinding stops the old lifecycle and it cannot grant progression or story outcomes. `ExaminerCombatPresenter`, `DivineThreatAura`, and `ExaminerActionSfx` remain presentation-only. The two source processors apply fixed scale/baseline normalization; the Descent processor chroma-removes V5 boards and mirrors the right profile for exact left-profile mass. Combat Lab strips rewards, caps the trial to one, and adds an idempotent debug-only phase-force control. Production Stage VII still requires a separate flow owner for Continue/Challenge, defeat interception, abnormal victory, rewards, replay, and saving.
