...
- The current Examiner implementation is deliberately contained by `levels/combat_lab/`: `Examiner` owns combat state, movement, hit timing, Refutation, and Axiom damage checks; `examiner_visual.gd` maps those states to named `AnimatedSprite2D` ranges; `ExaminerAxiomLane` and `CourtOfFirstMeasure` are presentation-only. Six action-owned sheets keep locomotion, thrust, sweep, dash, parry, and reactions independently replaceable. `tools/process_examiner_animation_sources.py` finds source gutters, applies one fixed scale, anchors the lower body to a shared foot line, derives right-facing motion by exact mirroring of the approved left row, and rejects edge clipping. Axiom deliberately composes sweep/dash body poses while its lane energy remains separate instead of importing cross-cell weapon fragments. Combat Lab strips rewards, caps the unique trial to one, swaps only the arena presentation layer, and provides no Stage VII transition or save authority.
