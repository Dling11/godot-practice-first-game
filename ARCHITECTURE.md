...
Decision 091 uses current health to select one of three repeatable jump tiers without presentation polling. At 80-100%, `_next_jump_chain_count()` returns one; below 80%, it alternates two/three; at 30% or lower, it cycles three/four/five. The first low-health crossing queues an immediate legal chain, while later chains obey post-prison reuse cooldown and melee gates. Each loop snapshots a new target using direct, velocity-lead, lateral-cutoff, opposite-side-cutoff, and direct-finisher policies as its count allows. Active timing/damage/recovery fields keep every tier on the same state machine, and only the final recovery sets `_root_ready`. `Stage5BossVisual` observes the low-health chain pulse and `Stage5BossLandingFeedback` reads controller-provided shake strength; neither owns targets, counts, or damage.
