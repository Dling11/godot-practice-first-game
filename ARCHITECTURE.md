...
- `AbilityComponent`: owns generic cast phases, snapshotted equipped-weapon damage, definition-owned hitbox selection, timed per-strike hitbox activation, instance-local cooldown state, and definition-opted invulnerability events. `AbilityDefinition` supplies stable identity, activation/presentation modes, flat-plus-weapon scaling, per-strike damage/knockback/stagger data, active movement speed, timings, shape, dash-cancel permission, and read-only UI metadata.
...
`SaveService` owns one versioned disk-profile boundary. It composes version-1 snapshots from `RunSession`, `StoryState`, `WeaponInventory`, `MaterialInventory`, and `RecipeDiscovery`, validates every nested version before mutation, restores those authorities only after complete validation, and records Sanctuary as the current safe scene. Material inventory and recipe discovery occupy their previously reserved `extensions` sections; stage claims and regional progress remain reserved empty dictionaries. Each authority owns its explicit versioned snapshot/restore contract and none performs file I/O. Pre-Segment-2 version-1 profiles with empty material/recipe extension dictionaries remain valid and reconstruct clean empty crafting state.
...
`MaterialInventory` owns mutable known-material quantities and validates saved IDs against the global catalog. `RecipeDiscovery` owns known recipe IDs separately from `StoryState` and validates them against the recipe catalog. Both emit observation signals, expose explicit versioned snapshots, reset on New Journey, and are reconstructed through `SaveService`; neither rolls rewards, performs file I/O, or crafts output. Crafting validation plus atomic ingredient consumption/output grant remain planned `CraftingService` responsibilities. Enemy controllers may later report a defeated enemy/drop-profile identity, encounter flow may request the authored clear chest, and UI may preview/request actions, but none may independently roll loot, mutate counts, evaluate recipes, or write save files.

Current profiles and loot tables are data-only. No controller references a drop profile, no stage resolves a `LootTableDefinition`, and the four recipe output IDs do not yet resolve to obtainable equipment. Segment 3 must add a single reward-resolution boundary plus idempotent first-clear claim state before enabling chest presentation; Segment 5 must add `CraftingService` and actual output integration before any recipe is described as craftable.

The planned Sanctuary Rootweaver/Grove Artificer is a presentation/service boundary separate from Armskeeper Orren. Its dialogue and craft menu observe recipe/inventory state and submit commands; it does not contain recipe balances in scene nodes. Stage-clear rewards must be idempotent at the save boundary so a reload cannot duplicate a one-time clear guarantee, while explicitly replayable Hunt rewards remain repeatable by authored rule.

Runtime material art belongs under `assets/items/materials/common/` and `assets/items/materials/<region>/`; reusable source templates belong under `art_source/generated/items/materials/templates/`. Template reuse is an art-production optimization only: every distinct material still resolves through its own definition and flattened runtime texture. Runtime recoloring, filename parsing, and label-driven gameplay logic are prohibited.
...
`SanctuaryFlow` composes the safe hub. It binds the existing player HUD, forwards contextual prompt metadata from two reusable `DialogueNpc` instances and `ExpeditionAltar`, and opens `DialoguePanel`, `CharacterMenu`, `WeaponShopMenu`, or `ExpeditionMenu`. Entering Sanctuary records `awakened_in_sanctuary`. Eira's completed dialogue opens `CharacterMenu.open_skillkeeper_menu()`; at Level 3 the selected second slot exposes `AWAKEN SKILL • FREE`, and `Player.awaken_skill_2()` records `opaw_consecutive_thrust_awakened` in `StoryState` before switching to the authored two-skill loadout. New Player instances restore that loadout from the flag. Orren's completed dialogue opens his weapon shop; Escape cancellation restores play without chaining into either menu. Each modal owns its temporary pause and restores it on close.
