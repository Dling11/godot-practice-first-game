...
`WeaponDefinition.damage` remains the weapon-scaled ability input; optional `basic_damage_minimum/maximum` supply one integer normal-hit roll, with zero values falling back to fixed `damage`. King's active `king_signature_sword.tres` owns 25 skill power, a 10-12 normal range, the 48-pixel-forward/28-pixel-half-width `king_sword_cleave.tres`, and `king_sword_form.tres`. Its `uses_integrated_visual` flag permits null world art because King's body frames already draw the sword; `PlayerWeaponVisual` hides the detached sprite while still observing trail phases. Ashwood/Iron remain Opaw-only `opaw_weapon_catalog.tres` content with their original Balanced Slash and fixed 25/32 damage. `king_weapon_catalog.tres` contains only King's signature sword. `WeaponInventory` registers defaults for both character IDs and adds King's default when restoring older version-1 saves. `Player.set_weapon_definition()` remains the idle-only synchronization seam; `Player.equip_owned_weapon()` validates against the active character catalog, ownership, and class compatibility. `MeleeHitbox` deduplicates contacts per swing, `HurtboxComponent` forwards `DamageInfo`, and `HealthComponent` owns healing, mitigation, damage, and death.
...
`EnemyRewardComponent` observes its own actor's `HealthComponent.died` event and grants the injected player XP/coins from its `EnemyDefinition`. Enemy definitions own reward values and `armor_rating`; every enemy controller copies armor into its own `HealthComponent` during setup. `HealthComponent` resolves accepted damage as `raw * 100 / (100 + armor)`, stores the raw value on `DamageInfo`, and emits the accepted value for feedback. Zero armor preserves legacy damage; Varkuun is the first nonzero profile at 30. `RunSession` retains XP, coins, and current player health across scene replacement; each new `PlayerProgressionComponent` reconstructs its level from the cumulative `0/150/400/750/1200/1750/2400/3150/4000/4950` thresholds. `PlayerVitalityComponent` resolves `140 + 12 * (level - 1) + equipment bonus`; player armor-item aggregation is still future work. `PlayerHealthRegenerationComponent` waits five damage-free seconds and requests 1 HP/s through `HealthComponent.heal()`. New journey and defeat restart clear the snapshot through `RunSession.reset_run()`.
