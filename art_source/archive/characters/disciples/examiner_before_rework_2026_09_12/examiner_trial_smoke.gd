...
func _run() -> void:
	...
	if (
		...
		or body.sprite_frames.get_animation_names().size() != 112
	):
	...
	if (
		...
		or thrust_left == null
		or walk_right.region.position.y != 256.0
		or walk_left.region.position.y != 128.0
		or thrust_right.region.position.y != 128.0
		or thrust_left.region.position.y != 256.0
	):
		_fail("Examiner locomotion and action sheets lost their independently authored left/right row mappings.")
		return
	var expected_frames := {
		&"idle_down": 2,
		&"walk_right": 4,
	...
	if (
		...
		or not axiom_down_contact.atlas.resource_path.ends_with("examiner_axiom_divide_sheet_192x128.png")
		or axiom_down_contact.region.position != Vector2(384.0, 0.0)
		or axiom_up_contact.region.position != Vector2(384.0, 384.0)
		or axiom_second_contact.region.position != Vector2(768.0, 0.0)
		or axiom_up_second_contact.region.position != Vector2(768.0, 384.0)
	):
	...
	for contact: AtlasTexture in down_axiom_contacts:
		...
		if (
			opaque_bounds.size.x > 58
			or absf(opaque_center_x - 96.0) > 8.0
			or opaque_bounds.end.y < 125
			or absf(weapon_tip_center_x - 96.0) > 6.0
		):
			_fail(
	...
	for contact: AtlasTexture in up_axiom_contacts:
		var opaque_bounds := _opaque_bounds(contact)
		var opaque_center_x := float(opaque_bounds.position.x) + float(opaque_bounds.size.x) * 0.5
		if opaque_bounds.size.x > 42 or absf(opaque_center_x - 96.0) > 8.0:
