...
func _run() -> void:
	...
	if controller.rootbound_husk_scene == null or controller.max_active_enemies != 4:
		...
		return
	if (
		controller.portal_target_scene != "res://levels/stage_4/stage_4.tscn"
		or controller.portal_tier != StagePortal.PortalTier.NORMAL
	):
		_fail("Stage 3's post-mini-boss portal must announce and continue into the normal Stage 4 route.")
		return
	if (
