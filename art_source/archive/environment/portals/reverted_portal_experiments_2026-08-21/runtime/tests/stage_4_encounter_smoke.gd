...
func _run() -> void:
	...
	if controller.armored_hog_scene == null:
		...
		return
	if (
		controller.portal_target_scene != "res://levels/stage_5/stage_5.tscn"
		or controller.portal_tier != StagePortal.PortalTier.BOSS
	):
		_fail("Stage 4 must announce and advance into the production Stage 5 boss route.")
		return
