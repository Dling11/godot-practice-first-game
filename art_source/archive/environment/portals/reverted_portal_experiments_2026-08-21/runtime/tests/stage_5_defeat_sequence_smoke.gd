...
func _run() -> void:
	...
	var effects := stage.get_node("World/Effects") as Node2D
	...
	if chest == null or chest.global_position != defeat_position:
		...
		return
	stage.call("_spawn_return_portal")
	await process_frame
	var return_portal := _find_stage_portal(effects)
	if return_portal == null or return_portal.portal_tier != StagePortal.PortalTier.NORMAL:
		_fail("Stage 5's Sanctuary return must use the Normal destination tier.")
		return
...
func _find_stage_portal(parent: Node) -> StagePortal:
	for child: Node in parent.get_children():
		if child is StagePortal:
			return child as StagePortal
	return null
