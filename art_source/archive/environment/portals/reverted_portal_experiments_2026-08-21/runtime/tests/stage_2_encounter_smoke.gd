...
func _run() -> void:
	...
	if controller.max_active_enemies != 6:
		...
		return
	if (
		controller.portal_target_scene != "res://levels/stage_3/stage_3.tscn"
		or controller.portal_tier != StagePortal.PortalTier.MINI_BOSS
	):
		_fail("Stage 2's clear portal must announce and continue into the Stage 3 mini-boss route.")
		return
	if (
