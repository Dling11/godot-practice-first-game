...
- Generated landmark architecture uses one fixed raster. Animate a separate cropped energy layer; never rely on near-matching full-structure generations. Architecture-free vortex sheets must preserve one footprint, baseline, scale, and cell size while only internal energy rotates. Interaction collision and scene-transition authority remain stable and code-owned.
