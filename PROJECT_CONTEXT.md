...
- UI: layered title screen with mouse plus keyboard/gamepad focus and session-audio settings, plus the shared dark-fantasy theme and named pixel icons across combat HUD, character menu, contextual portal prompt, wave messages, and enemy health bars. The HUD and character menu build their four slots from one reusable skill-loadout definition. Gameplay modals expose pointer controls, directional/Enter navigation, and Escape cancellation; Tab opens the character menu.
